...
function buildTasksMd(doneLines) {
    const donePart = doneLines.length === 1
        ? `**done**: ${doneLines[0]}`
...
describe("validateTaskDone", () => {
    ...
    function setup(doneLines, extraFiles = {}) {
    ...
    // ── coverage: ────────────────────────────────────────────────────────────
    function makeIstanbulReport(entries) {
        const report = {};
        let grandTotal = 0;
        let grandCovered = 0;
    ...
    function makePytestReport(files, totalPct) {
        const filesObj = {};
    ...
    it("coverage: — Istanbul 리포트, 파일 경로 지정, 임계값 통과", async () => {
        ...
        try {
            ...
            writeFileSync(join(tmpDir, "coverage", "coverage-summary.json"), makeIstanbulReport({ [srcFile]: { total: 100, covered: 85 } }));
    ...
    it("coverage: — Istanbul 리포트, 디렉터리 경로 집계, 임계값 통과", async () => {
        ...
        try {
            ...
            writeFileSync(join(tmpDir, "coverage", "coverage-summary.json"), makeIstanbulReport({ [f1]: { total: 50, covered: 40 }, [f2]: { total: 50, covered: 30 } }));
    ...
    it("coverage: — Istanbul 리포트, 임계값 미달", async () => {
        ...
        try {
            ...
            writeFileSync(join(tmpDir, "coverage", "coverage-summary.json"), makeIstanbulReport({ [srcFile]: { total: 100, covered: 70 } }));
            writeFileSync(join(tmpDir, "tasks.md"), buildTasksMd([`coverage: src/api.ts :: 80`]));
            const result = await validateTaskDone({ task_id: "T1-001" });
            expect(result.passed).toBe(false);
    ...
    it("coverage: — Istanbul 리포트, 경로 없을 때 total fallback", async () => {
        ...
        try {
            ...
            writeFileSync(join(tmpDir, "coverage", "coverage-summary.json"), makeIstanbulReport({ [otherFile]: { total: 100, covered: 90 } }));
    ...
    it("coverage: — pytest-cov 리포트, 임계값 통과", async () => {
        ...
        try {
            ...
            writeFileSync(join(tmpDir, "coverage.json"), makePytestReport({ [pyFile]: { statements: 50, covered: 45 } }, 90));
    ...
    it("coverage: — pytest-cov 리포트, 임계값 미달", async () => {
        ...
        try {
            ...
            const pyFile = join(tmpDir, "backend", "app", "users.py");
            writeFileSync(pyFile, "");
            writeFileSync(join(tmpDir, "coverage.json"), makePytestReport({ [pyFile]: { statements: 50, covered: 30 } }, 60));
            writeFileSync(join(tmpDir, "tasks.md"), buildTasksMd([`coverage: backend/app :: 80`]));
            const result = await validateTaskDone({ task_id: "T1-001" });
            expect(result.passed).toBe(false);
            expect(firstReason(result)).toContain("미달");
    ...
    it("coverage: % 기호 있어도 파싱 성공", async () => {
        ...
        try {
            ...
            writeFileSync(srcFile, "");
            writeFileSync(join(tmpDir, "coverage", "coverage-summary.json"), makeIstanbulReport({ [srcFile]: { total: 10, covered: 10 } }));
