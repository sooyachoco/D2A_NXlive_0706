...
# 상위 워크플로 단계 — 큰 흐름(전체 파악용). PROGRESS.md Phase A~H를 6단계로 압축.
FLOW_STEPS = ["설치", "세팅", "명세", "구현", "점검", "배포"]


def compute_flow(state, has_tasks, pct, total, stage_text, has_spec, has_frontend):
    """state.json + 파일 존재로 현재 '큰 단계'를 판정해 [(이름, 상태)] 반환.

    상태: done / current / todo. PROGRESS.md '현재 단계' 텍스트가 있으면 우선 보정.
    """
    auth = state.get("auth_profile")
    phase = state.get("phase")
    setup_done = bool(auth) or has_frontend or (phase is not None) or has_spec or has_tasks
    spec_done = has_tasks  # spec→plan→tasks 순서: tasks.md 생성 = 명세 완료
    impl_done = total > 0 and pct >= 100
    done = {
        "설치": True,           # 이 스크립트가 도는 시점 = 설치 완료
        "세팅": setup_done,
        "명세": spec_done,
        "구현": impl_done,
        "점검": False,          # 자동 완료 신호 없음 (사람이 수동 진행)
        "배포": False,
    }
    current = next((n for n in FLOW_STEPS if not done[n]), FLOW_STEPS[-1])

    # PROGRESS.md '현재 단계' 텍스트로 보정 (source of truth 우선)
    st = stage_text or ""
    if "배포" in st:
        current = "배포"
    elif any(k in st for k in ("점검", "보안", "품질")):
        current = "점검"

    idx = FLOW_STEPS.index(current)
    return [
        (n, "done" if i < idx else ("current" if i == idx else "todo"))
        for i, n in enumerate(FLOW_STEPS)
    ]
...
def render(state, phases, progress, activity, project_name, activity_path=None):
    ...
    # ── 상위 워크플로 레일 (큰 흐름 — 전체 파악) ──
    has_spec = bool(_first_glob(os.path.join(ROOT, "specs", "*", "spec.md")))
    has_frontend = os.path.isdir(os.path.join(ROOT, "frontend"))
    flow = compute_flow(state, bool(phases), pct, total, progress.get("stage"), has_spec, has_frontend)
    flow_parts = []
    for i, (fname, fstate) in enumerate(flow):
        if i > 0:
            flow_parts.append('<span class="flow-sep"></span>')
        flow_parts.append(f'<span class="flow-step flow-{fstate}">{esc(fname)}</span>')
    flow_html = f'<div class="flow-rail">{"".join(flow_parts)}</div>'
...
/* 상위 워크플로 레일 — 큰 흐름(전체 파악). 현재 단계 강조 */
.flow-rail {{ display:flex; align-items:center; flex-wrap:wrap; gap:2px;
  border-top:1.5px solid var(--text); padding-top:13px; margin-bottom:26px; }}
.flow-step {{ font-size:12px; font-weight:700; text-transform:uppercase; letter-spacing:0.05em;
  color:var(--muted-soft); padding:4px 11px; white-space:nowrap; }}
.flow-step.flow-done {{ color:var(--text); }}
.flow-step.flow-done::before {{ content:'\\2713 '; color:var(--green); }}
.flow-step.flow-current {{ color:var(--accent); border:1.5px solid var(--accent); }}
.flow-sep {{ flex:0 0 auto; width:16px; height:1.5px; background:var(--border-soft); }}
