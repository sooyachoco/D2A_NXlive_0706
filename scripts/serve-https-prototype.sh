#!/bin/bash
# [DEPRECATED 2026-06] Phase 0.6/0.7 전용 HTTPS 프로토타입 서버
#
# ⚠️ 이 스크립트는 dev 서버를 sudo 로 443 에 직접 바인딩하여 Caddy 게이트키퍼 모델과 충돌한다.
#    표준 흐름(setup-https.sh + create-spec Step 2.7 / save-auth-state.sh)을 사용하라.
#    실행하려면 ALLOW_LEGACY_443_PROTOTYPE=1 옵트인이 필요하다 (본문 가드 참조).
#
# 사용 목적:
...
# 이 스크립트는 dev 서버를 sudo 로 443 에 직접 바인딩한다 — 보일러플레이트의 Caddy 게이트키퍼
# 모델(Caddy 단일 데몬이 443 점유 + SNI 분기, dev 는 8010+ 평문 HTTP)과 정면으로 충돌하며,
# 한 번 띄워두면 그 자체가 "443 점유 외부 프로세스"가 되어 setup-https.sh 를 깨뜨린다.
# 표준 흐름: setup-https.sh(HTTPS 셋업) + create-spec Step 2.7 / save-auth-state.sh(로그인·검증).
if [[ "${ALLOW_LEGACY_443_PROTOTYPE:-}" != "1" ]]; then
  cat <<EOF >&2

⚠️  serve-https-prototype.sh 는 DEPRECATED 입니다 (2026-06).
    dev 서버를 sudo 로 443 에 직접 올리는 방식은 Caddy 게이트키퍼 모델과 충돌합니다.

  권장 대안:
    ./scripts/setup-https.sh $DOMAIN frontend   # Caddy 가 443 종단, dev 는 8010+ 평문
    (UI 프로토타입·로그인 검증은 create-spec Step 2.7 / save-auth-state.sh)

  그래도 레거시 동작이 꼭 필요하면 명시적으로 옵트인:
    ALLOW_LEGACY_443_PROTOTYPE=1 sudo $0 $DOMAIN $BACKEND_PORT

EOF
  exit 1
fi

# 옵트인했더라도 Caddy 등이 이미 443 을 점유 중이면 직접 바인딩은 충돌 → 사전 차단
if netstat -an -p tcp 2>/dev/null | grep -qE '[:.]443[[:space:]].*LISTEN'; then
  cat <<EOF >&2

❌ 443 포트가 이미 점유 중입니다 — 이 레거시 서버는 443 을 직접 바인딩하므로 충돌합니다.
   점유 주체 확인:
     sudo lsof -nP -iTCP:443 -sTCP:LISTEN -t | xargs -I{} ps -o pid=,user=,comm= -p {}
   Caddy(게이트키퍼)라면 이 스크립트 대신 setup-https.sh 흐름을 사용하세요.
