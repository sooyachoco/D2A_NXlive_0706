...
#   사전 정리: 좀비 system plist + user-level LaunchAgent + root 변질된 caddy 경로 자동 감지
#   TCC 회피: 인증서가 ~/Desktop·~/Documents·~/Downloads 에 있으면 launchd 가 파일 접근 차단 →
#             해당 경우 1순위(brew services)를 건너뛰고 직접 실행으로 자동 우회
#   443 탐지: netstat(소유자 무관)로 LISTEN 존재 확인 후 lsof 로 소유자 식별 — 소유자 불명 시 강행 금지
#
# 주의:
...
# .env 파일에 KEY=VALUE 를 upsert (있으면 교체, 없으면 추가)
# 재실행 시 도메인/포트가 바뀌어도 stale 값이 남지 않도록 한다.
upsert_env() {
  local file="$1" key="$2" val="$3"
  if grep -qE "^${key}=" "$file" 2>/dev/null; then
    local tmp
    tmp="$(mktemp -t d2a-env.XXXXXX)"
    # 구분자 | 사용(값에 / 가능). 호출부 값은 상위 화이트리스트 통과(영숫자·점·하이픈/숫자)라 & · | 불가.
    sed "s|^${key}=.*|${key}=${val}|" "$file" > "$tmp"
    # mv 대신 cat>로 기존 파일에 덮어쓴다 — mktemp 의 0600 권한이 전파되거나 심링크가 끊기지 않도록
    # 원본 파일의 권한·inode·심링크 타깃을 보존한다.
    cat "$tmp" > "$file"
    rm -f "$tmp"
  else
    printf '%s=%s\n' "$key" "$val" >> "$file"
...
if [[ ! -f "$CADDYFILE" ]]; then
  # 사이트 블록마다 명시 tls 를 발급하므로 local_certs(내부 CA)는 불필요.
  cat > "$CADDYFILE" <<EOF
{
  auto_https off
}
...
# ── macOS TCC 위험 감지 ──
# launchd(brew services)로 띄운 Caddy 는 ~/Desktop · ~/Documents · ~/Downloads 영역의
# 파일 접근이 TCC(System Integrity)로 차단된다. 인증서가 그 영역에 있으면 brew services 가
# "started" 직후 error 로 전환되므로, 그 경우 직접 실행(caddy start)을 강제한다.
TCC_RISK=false
if [[ "$OSTYPE" == "darwin"* ]]; then
  case "$CERT_OUT/" in
    "$HOME/Desktop/"*|"$HOME/Documents/"*|"$HOME/Downloads/"*) TCC_RISK=true ;;
  esac
fi
...
# TCC_RISK=true (인증서가 보호 영역) → 1순위 건너뛰고 직접 실행 (launchd TCC 차단 회피)
if [[ "$OSTYPE" == "darwin"* ]]; then
  # ── 443 점유 탐지 (소유자 무관 우선) ──
  # netstat 는 sudo 없이도 소유자와 무관하게 LISTEN 소켓 존재를 보여준다.
  # non-sudo lsof 는 다른 사용자(root) 소유 소켓을 못 보므로 "비어있음" 오판의 원인 →
  # 반드시 netstat 로 먼저 존재를 확인하고, 소유자 식별만 lsof 로 한다.
  PORT_443_LISTENING=false
  if netstat -an -p tcp 2>/dev/null | grep -qE '[:.]443[[:space:]].*LISTEN'; then
    PORT_443_LISTENING=true
  fi

  # 소유자 식별: 비특권 lsof → (못 보면) sudo -n lsof 에스컬레이션
  PORT_443_OWNER=$(lsof -nP -iTCP:443 -sTCP:LISTEN 2>/dev/null \
    | awk 'NR>1 {print $1}' | sort -u | head -3 | tr '\n' ',' | sed 's/,$//' || true)
  if [[ -z "$PORT_443_OWNER" && "$PORT_443_LISTENING" == "true" ]]; then
  ...
  elif [[ "$PORT_443_LISTENING" == "true" && -z "$PORT_443_OWNER" ]]; then
    # 리스너는 있으나 소유자 식별 실패 (sudo 캐시 없음 등) — 강행 금지
    cat <<EOF >&2

ERROR: 443 포트에 LISTEN 중인 프로세스가 있으나 소유자를 식별하지 못했습니다.
  (비특권 lsof 로는 다른 사용자(root) 소유 소켓이 보이지 않습니다)

소유자 확인 (sudo 필요):
  sudo lsof -nP -iTCP:443 -sTCP:LISTEN -t | xargs -I{} ps -o pid=,user=,comm= -p {}

확인 후:
  - Caddy 라면      : 이 스크립트를 다시 실행하면 자동 재사용합니다.
  - 다른 프로세스라면 : 종료(sudo kill <PID>)하거나 해당 dev 도구를 멈춘 뒤 재실행하세요.
...
EOF
    ...
    # ── 사전 정리 ①-b 좀비 user-level LaunchAgent (최신 Homebrew brew services 경로) ──
    # ~/Library/LaunchAgents/homebrew.mxcl.caddy.plist 가 error/spawn-scheduled 상태면
    # launchd 가 caddy 를 재시작 루프에 올려 443 충돌을 반복시킨다. system plist 만 정리하면
    # 이 경로가 남아 brew services 재실행 시 좀비가 재등록된다. (443 비점유 상태에서만 도달)
    USER_CADDY_PLIST="${HOME:-}/Library/LaunchAgents/homebrew.mxcl.caddy.plist"
    # HOME 미설정 시 시스템 경로(/Library/...)로 떨어지는 것을 방지 — user-level 정리만 수행
    if [[ -n "${HOME:-}" && -f "$USER_CADDY_PLIST" ]]; then
      BREW_CADDY_STATE=$(brew services list 2>/dev/null | awk '$1=="caddy"{print $2}' || true)
      USER_AGENT_BAD=false
      [[ "$BREW_CADDY_STATE" == "error" ]] && USER_AGENT_BAD=true
      if launchctl print "gui/$(id -u)/homebrew.mxcl.caddy" 2>/dev/null \
           | grep -qiE 'spawn scheduled|last exit code = [^0]'; then
        USER_AGENT_BAD=true
      fi
      if [[ "$USER_AGENT_BAD" == "true" ]]; then
        echo "      ⚠️  user-level caddy LaunchAgent 좀비 감지 — 정리 시도"
        brew services stop caddy 2>/dev/null || true
        launchctl bootout "gui/$(id -u)/homebrew.mxcl.caddy" 2>/dev/null || true
        rm -f "$USER_CADDY_PLIST" 2>/dev/null || true
        echo "      → user LaunchAgent 정리 완료"
      fi
    fi
...
MSG
    ...
    started=false
    start_log=""
    # ⚠️ caddy start 는 데몬을 fork 하고 자식이 stdout/stderr(파이프 write-end)를 계속 물기 때문에
    #    $(caddy start ...) 명령 치환은 EOF 를 못 받아 영원히 hang 한다.
    #    → 명령 치환 대신 임시 파일 리다이렉트로 회피한다 (파일 fd 는 부모 셸을 막지 않음).
    START_LOG_FILE="$(mktemp -t d2a-caddy-start.XXXXXX)"
    trap 'rm -f "$START_LOG_FILE"' EXIT

    if [[ "${CADDY_MODE:-}" != "direct" && "$TCC_RISK" != "true" ]]; then
      echo "      Caddy 시작 시도 [1/2]: brew services start (user-level, sudo 없음)"
      if brew services start caddy >"$START_LOG_FILE" 2>&1; then
        start_log="$(cat "$START_LOG_FILE" 2>/dev/null || true)"
        sleep 2
    ...
    elif [[ "$TCC_RISK" == "true" ]]; then
      cat <<EOF
      ⚠️  TCC 경고: 인증서 경로가 보호 영역에 있습니다
          $CERT_OUT
          launchd/brew services 로 띄운 Caddy 는 이 경로의 인증서를 읽지 못해 "started"
          직후 error 로 전환됩니다. → brew services 를 건너뛰고 직접 실행으로 우회합니다.
          (영구 회피: 인증서를 ~/Desktop·~/Documents·~/Downloads 밖으로 옮기거나 프로젝트를 이동)
EOF
    else
    ...
    if [[ "$started" == "false" ]]; then
      ...
      # 명령 치환 대신 파일 리다이렉트 — fork 된 데몬이 fd 를 물어도 hang 하지 않는다.
      if caddy start --config "$CADDYFILE" --adapter caddyfile >"$START_LOG_FILE" 2>&1; then
        start_log="$(cat "$START_LOG_FILE" 2>/dev/null || true)"
        sleep 2
...
if caddy validate --config "$CADDYFILE" --adapter caddyfile 2>/dev/null; then
  if caddy reload --config "$CADDYFILE" --adapter caddyfile 2>/dev/null; then
    ...
    # Linux: 데몬 시작/리로드는 이 스크립트가 자동화하지 않는다 (systemd 등 OS 별 방식)
    echo "      ⚠️  reload 실패 (Linux) — Caddy 데몬을 수동 기동/리로드하세요:"
    echo "          systemctl reload caddy   또는   caddy reload --config $CADDYFILE"
  fi
else
...
# ── 최종 검증 (smoke test) ─────────────────────────────────────
# "Caddy 는 떴지만 빈 설정/인증서 미접근(TCC)" 같은 up-but-broken 상태를 출구에서 잡는다.
# dev 서버가 아직 안 떠 있으면 reverse_proxy 가 502 를 주는데, 이는 Caddy/TLS 가 정상이라는 신호다.
# HTTP 000 (연결/TLS 실패) 만 진짜 실패로 본다.
echo "[검증] https://$DOMAIN TLS 응답 확인..."
# curl 은 연결/TLS 실패 시 이미 stdout 에 "000" 을 쓰고 비0 으로 종료한다.
# 따라서 $(...) 안에서 '|| echo 000' 을 붙이면 "000000" 이 되어 실패가 정상으로 빠진다.
# → 명령 치환 밖에서 '|| SMOKE_CODE=000' 으로 set -e 차단 + 값 정규화한다.
SMOKE_CODE="$(curl -ks -o /dev/null -w '%{http_code}' --max-time 5 \
  --resolve "$DOMAIN:443:127.0.0.1" "https://$DOMAIN" 2>/dev/null)" || SMOKE_CODE="000"
if [[ "$SMOKE_CODE" == "000" ]]; then
  cat <<EOF
      ⚠️  smoke test 실패 — https://$DOMAIN 응답 없음 (HTTP 000)
          가능 원인:
            ① Caddy 가 인증서를 못 읽음 (TCC — 위 경고 참조)
            ② Caddy 데몬 미기동 — 확인: pgrep -x caddy
            ③ 인증서/hosts 미반영
          진단: tail -20 /opt/homebrew/var/log/caddy.log
EOF
else
  echo "      → TLS 응답 코드 $SMOKE_CODE (dev 서버 미기동 시 502 정상 — Caddy/TLS 자체는 동작)"
fi
...
if [[ -n "$ENV_EXAMPLE" ]]; then
  ...
  if ! grep -qE '^LOCAL_DEV_HOST=' "$ENV_EXAMPLE"; then
    # 신규 — 주석 헤더와 함께 추가
    {
      echo ""
    ...
    echo "      → $ENV_EXAMPLE 갱신(신규): LOCAL_DEV_HOST=$DOMAIN, LOCAL_DEV_PORT=$DEV_PORT, LOCAL_BACKEND_PORT=$BE_PORT"
  else
    # 기존 값 동기화 — 재실행 시 도메인/포트가 바뀌어도 stale 값이 남지 않도록 upsert
    upsert_env "$ENV_EXAMPLE" LOCAL_DEV_HOST "$DOMAIN"
    upsert_env "$ENV_EXAMPLE" LOCAL_DEV_PORT "$DEV_PORT"
    upsert_env "$ENV_EXAMPLE" LOCAL_BACKEND_PORT "$BE_PORT"
    echo "      → $ENV_EXAMPLE 동기화: LOCAL_DEV_HOST=$DOMAIN, LOCAL_DEV_PORT=$DEV_PORT, LOCAL_BACKEND_PORT=$BE_PORT"
  fi
fi
