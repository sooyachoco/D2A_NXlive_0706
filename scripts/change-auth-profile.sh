...
if [[ "$CURRENT_HOST" != "(미설정)" ]] && [[ "$CURRENT_HOST" != "(state.json 없음)" ]]; then
  # Caddyfile 위치 자동 감지 (setup-https.sh 와 동일 후보 — brew --prefix 우선)
  CADDYFILE=""
  CADDY_CANDIDATES=()
  if command -v brew &>/dev/null; then
    CADDY_CANDIDATES+=("$(brew --prefix 2>/dev/null)/etc/Caddyfile")
  fi
  CADDY_CANDIDATES+=(/opt/homebrew/etc/Caddyfile /usr/local/etc/Caddyfile /etc/caddy/Caddyfile)
  for p in "${CADDY_CANDIDATES[@]}"; do
    if [[ -f "$p" ]]; then
      CADDYFILE="$p"
      break
    fi
  ...
  if [[ -n "$CADDYFILE" ]]; then
    ...
    if [[ "$CURRENT_HOST" =~ ^[a-zA-Z0-9]([a-zA-Z0-9.-]*[a-zA-Z0-9])?$ ]] \
      ...
      rm -f "$SITE_FILE"
      echo "      → 삭제: $SITE_FILE"
      # Caddy reload — admin API(localhost:2019) 우선. sudo brew services 는 caddy 바이너리
      # 경로를 root 소유로 변질시키므로 사용하지 않는다 (setup-https.sh 와 동일 정책).
      if caddy reload --config "$CADDYFILE" --adapter caddyfile 2>/dev/null; then
        echo "      → Caddy reload 완료 (admin API)"
      elif [[ "$OSTYPE" == "darwin"* ]] && brew services reload caddy 2>/dev/null; then
        echo "      → Caddy reload 완료 (brew services, user-level)"
      else
        echo "      ⚠️  Caddy reload 실패 — 수동: caddy reload --config $CADDYFILE"
      fi
    else
