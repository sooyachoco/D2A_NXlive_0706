...
| 시각 대시보드 | `.claude/status.html` | 전체 흐름·현재 단계 시각화. stop hook이 매 응답 종료 시 자동 갱신. 읽기 전용·`.gitignore` 대상. |
...
**진행 현황 대시보드 안내 (status.html)**

`.claude/status.html`은 전체 작업 흐름(설치→세팅→명세→구현→점검→배포)과 현재 단계를 한눈에
보여주는 읽기 전용 대시보드다 (stop hook이 매 응답 종료 시 자동 갱신). 목적은 **① 큰 흐름에서
전체 단계 파악 ② 중간중간 현재 단계 인지**.

AI는 **단계가 전환되는 시점에만** 사용자에게 현재 위치와 대시보드 링크를 한 줄로 환기한다
(일상 응답에는 붙이지 않는다 — 노이즈 방지):

| 환기 시점 | 안내 형식 |
|---|---|
| Phase 시작/완료 | `📍 [구현] N/M단계 · {이름} {시작\|완료} — 전체 현황: .claude/status.html` |
| 블로커 발생/해제 | `📍 막힌 작업 {발생\|해제} — 현황: .claude/status.html` |
| 세션 복귀("이어서 해줘") | session-phase-workflow 복귀 안내 끝에 대시보드 링크 동봉 |

최초 인지는 `boilerplate-setup` 또는 `create-spec` 완료 시 1회 소개한다
("진행 현황은 `.claude/status.html`에서 항상 확인"). 링크는 VSCode/Claude Code에서 클릭 시 열린다.
...
| `.claude/skills/` | 슬래시커맨드 스킬 파일 (25개) |
...
| `/design-handoff` | 파이프라인 종착 — UT 통과 후 개발 핸드오프 스펙 생성 (design:design-handoff 플러그인 호출) → HANDOFF.md |
